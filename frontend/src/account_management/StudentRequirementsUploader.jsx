...
const StudentRequirementsUploader = () => {
  ...
  const fetchUploadsByStudentNumber = async (student_number) => {
    ...
    try {
      const res = await axios.get(
        `${API_BASE_URL}/student/uploads/by-student/${student_number}`,
      );
      setUploads(res.data);
    } catch (err) {
  ...
  const handleStatusChange = async (uploadId, remarkValue) => {
    ...
    try {
      await axios.put(
        `${API_BASE_URL}/student/uploads/status/${uploadId}`,
  ...
  const handleDelete = async (uploadId) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/student/uploads/${uploadId}`, {
        headers: {
          "x-person-id": localStorage.getItem("person_id"),
          ...getAuditConfig().headers,
        },
